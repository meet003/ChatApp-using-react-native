import { child, getDatabase, get, push, ref, update, set, remove } from "firebase/database";
...
export const removeUserFromChat = async (userLoggedInData, userToRemoveData, chatData) => {
    ...
    await updateChatData(chatData.key, userLoggedInData.userId, { users: newUsers }); //main line of  reove user from chat
    ...
    for (const key in userChats) {
        ...
        if (currentChatId === chatData.key) {
            await deleteUserChat(userToRemoveId, key);// main line remove user from chat
            break;
        }
    }
