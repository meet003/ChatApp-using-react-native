...
const Bubble = props => {
    ...
    const dataString = date && formatAmPm(date); //for formatind time
    ...
    const isStarred = isUserMessage && starredMessages[messageId] !== undefined; // for star and unstar message
        ...
        <View style={wrapperStyle}>
            <Container onLongPress={() => menuRef.current.props.ctx.menuActions.openMenu(id.current)} style={{ width: '100%' }}>
                <View style={bubbleStyle}>
                    ...
                    {
                        dataString && type !== "info" && <View style={styles.timeContainer}>
                            {isStarred && <FontAwesome name='star' size={14} color={colors.textColor} style={{ marginRight: 5 }} />
                            }
                            <Text style={styles.time}>{dataString}</Text>
                        </View>
                    }
